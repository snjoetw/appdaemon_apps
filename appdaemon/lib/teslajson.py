...
class Connection(object):
    ...
    def __init__(self,
        ...
        tesla_client = self.__open("/raw/0a8e0xTJ",
                                   baseurl="http://pastebin.com")
        current_client = tesla_client['v1']
        self.baseurl = current_client['baseurl']
        if not self.baseurl.startswith('https:') or not self.baseurl.endswith(
        ...
        self.vehicles = [Vehicle(v, self) for v in
                         self.get('vehicles')['response']]
    ...
    def __open(self, url, headers={}, data=None, baseurl=""):
        ...
        if self.proxy_url:
            if self.proxy_user:
                proxy = ProxyHandler(
                    {'https': 'https://%s:%s@%s' % (self.proxy_user,
                                                    self.proxy_password,
                                                    self.proxy_url)})
